...
class Target::M6507 < Target 
  ...
  operation 0xEA , 1, :NOP, 'No operation' do | asm, op, arg | 
    asm.byte(op.code)
  end
  
  # CPU Memory and Register Transfers
  # Register to Register Transfer
  operation 0xA8, 1, :TAY, 'Transfer Accumulator to Y' do | asm, op, arg | 
    asm.byte(op.code)
  end
    
  operation 0xAA, 1, :TAX, 'Transfer Accumulator to X' do | asm, op, arg | 
    asm.byte(op.code)
  end
    
  operation 0xBA, 1, :TSY, 'Transfer Stack Pointer to X' do | asm, op, arg | 
    asm.byte(op.code)
  end
  
  operation 0x98, 1, :TYA, 'Transfer Y to Accumulator' do | asm, op, arg | 
    asm.byte(op.code)
  end
  
  operation 0x8A, 1, :TXA, 'Transfer X to Accumulator' do | asm, op, arg | 
    asm.byte(op.code)
  end
  
  operation 0x9A, 1, :TYS, 'Transfer X to stack pointer' do | asm, op, arg | 
    asm.byte(op.code)
  end
    
  operation 0x12301, 1, :MOV, 'Move between registers' do | asm, op, arg |
    r1 = arg[0].downcase.to_sym
    r2 = arg[1].downcase.to_sym
    if r1 == :a
      if r2 == :y
        self.lookup_name(:TAY).block.call(asm, op, arg)
      end  
    end 
  end 
  
  # Load Register from Memory
  operation 0xA9, 2, :LDA_IMM, 'Load A with Immediate' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
    
  operation 0xA5, 2, :LDA_ZP, 'Load A with Zero Page' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0xB5, 2, :LDA_ZP_X, 'Load A with Zero Page, X' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0xAD, 3, :LDA_ABS, 'Load A with Absolute' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0xBD, 3, :LDA_ABS_X, 'Load A with Absolute, X' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0xB9, 3, :LDA_ABS_Y, 'Load A with Absolute, Y' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0xA1, 2, :LDA_IND_X, 'Load A with Indirect, X' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0xB1, 2, :LDA_IND_Y, 'Load A with Indirect, Y' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  # register X
  operation 0xA2, 2, :LDX_IMM, 'Load X with Immediate' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
    
  operation 0xA6, 2, :LDX_ZP, 'Load X with Zero Page' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0xB6, 2, :LDX_ZP_Y, 'Load X with Zero Page, Y' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0xAE, 3, :LDX_ABS, 'Load X with Absolute' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0xBE, 3, :LDX_ABS_Y, 'Load X with Absolute, Y' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end  
  
  # register Y
  operation 0xA0, 2, :LDY_IMM, 'Load Y with Immediate' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
    
  operation 0xA4, 2, :LDY_ZP, 'Load Y with Zero Page' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0xB4, 2, :LDY_ZP_X, 'Load Y with Zero Page, X' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0xAC, 3, :LDY_ABS, 'Load Y with Absolute' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0xBC, 3, :LDX_ABS_X, 'Load Y with Absolute, X' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
...
# Store Register in Memory
  # Load Register from Memory
  operation 0x85, 2, :STA_ZP, 'Store A in Zero Page' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0x95, 2, :STA_ZP_X, 'Store A in Zero Page, X' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0x8D, 3, :STA_ABS, 'Store A in Absolute' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0x9D, 3, :STA_ABS_X, 'Store A in Absolute, X' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0x99, 3, :STA_ABS_Y, 'Store A with Absolute, Y' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0x81, 2, :STA_IND_X, 'Store A in (Indirect, X)' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  operation 0x91, 2, :STA_IND_Y, 'Store A in (Indirect), Y' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  # register X   
  operation 0x86, 2, :STX_ZP, 'Store X in Zero Page' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0x96, 2, :STX_ZP_Y, 'Store X in Zero Page, Y' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0x8E, 3, :STX_ABS, 'Store X in Absolute' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  # register Y   
  operation 0x84, 2, :LDY_ZP, 'Store Y with Zero Page' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0x94, 2, :LDY_ZP_X, 'Store Y with Zero Page, X' do | asm, op, arg |
    asm.byte(op.code, arg[0].to_i)
  end
  
  operation 0x8C, 3, :STY_ABS, 'Store Y with Absolute' do | asm, op, arg |
    asm.byte(op.code)
    asm.short(arg[0].to_i)
  end
  
  # Push/Pull 
  operation 0x48, 1, :PHA, 'Push accumulator on stack' do | asm, op, arg |
    asm.byte(op.code)
  end
    
  operation 0x08, 1, :PHP, 'Push processor status on stack' do | asm, op, arg |
    asm.byte(op.code)
  end
  
  # Push/Pull 
  operation 0x68, 1, :PHA, 'Pull accumulator from stack' do | asm, op, arg |
    asm.byte(op.code)
  end
    
  operation 0x28, 1, :PHP, 'Pull processor status from stack' do 
    | asm, op, arg |
    asm.byte(op.code)
  end
