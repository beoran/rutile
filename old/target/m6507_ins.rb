code = "#\n"
code << "# Mos 6507 CPU target.\n"
code << "\n"
code << "class Target::M6507 < Target\n"
DATA.each do |line|
  line = line.strip
  next if line =~ /\A#/  
  next if line !~ /\A[0-9A-F]{2}/
  codeinfo = line[0..7]
  # p codeinfo  
  opcode   = codeinfo[0..1].to_i(16)
  size     = 1
  size     = 2 if codeinfo.strip =~ /(nn|dd|disp)/
  size 	   = 3 if codeinfo.strip =~ /nn nn/
  time     = line[18..21].strip
  name     = line[22..32].strip
  name.gsub!(' #nn', '_IMM')
  name.gsub!(' nnnn', '_ABS')
  name.gsub!(' nn', '_ZP')
  name.gsub!(' (nn,X)', '_IND_X')
  name.gsub!(' (nn),Y', '_IND_Y')
  name.gsub!(',X', '_X')
  name.gsub!(',Y', '_Y')
  name.gsub!(' (nnnn)', '_IND')
  name.gsub!(' disp', '_DISP')
  comment   = line[33..100].strip.gsub!(/[ ]+/, ' ')  
  opcodeh   = "%x" % opcode
  # printf("%d %s %d %s %s %s\n", opcode, opcodeh, size, timing, name, comment)
  code << "  operation 0x#{opcodeh}, #{size}, :#{name}, '#{time}', \'#{comment}\' do | asm, op, arg |\n"
  code << "    asm.byte(op.code)\n"
  code << "    asm.byte(arg[0])\n" if size == 2
  code << "    asm.short(arg[0])\n" if size == 3
  code << "  end\n\n"
end
  code << "end\n\n"
  puts code
...
# JMP [03FFh] would fetch the MSB from [0300h] instead of [0400h]. Very simple 
# workaround would be to place a ALIGN 2 before the data word.

# Conditional Branches (Branch on condition, to PC=PC+/-nn)

  10 dd     ------  2** BPL disp    Branch On Plus             ;N=0 (plus/positive)
  30 dd     ------  2** BMI disp    Branch on Minus            ;N=1 (minus/signed)
  50 dd     ------  2** BVC disp    Branch on oVerflow Clear   ;V=0 (no overflow)
  70 dd     ------  2** BVS disp    Branch on oVerflow Set     ;V=1 
  90 dd     ------  2** BCC disp    Branch on Carry Clear      ;C=0 
  B0 dd     ------  2** BCS disp    Branch on Carry Set        ;C=1
  D0 dd     ------  2** BNE disp    Branch on Not Equal        ;Z=0 
  F0 dd     ------  2** BEQ disp    Branch on Equal            ;Z=1 (zero/equal)

# ** The execution time is 2 cycles if the condition is false (no branch executed). 
# Otherwise, 3 cycles if the destination is in the same memory page, or 4 cycles if it 
# crosses a page boundary (see below for exact info).
...
# return address from BRK skips one dummy/parameter byte following after the BRK opcode.
# Software or hardware must take care to acknowledge or reset /IRQ or /NMI signals 
# after processing it.
